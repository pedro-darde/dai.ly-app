...
export default {
  ...
  methods: {
    ...
    tasksToInsertAndToRemove() {
      ...
      const toInsert = this.note.tasks?.filter(
        (task) => !this.currentNote?.tasks?.includes(task)
      );
      const toRemove = this.currentNote?.tasks?.filter(
        (task) => !this.note.tasks?.includes(task)
      );
      return {
